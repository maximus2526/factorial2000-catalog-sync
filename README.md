...
- PHP 7.4 або новіше
- Права на запис у тимчасову директорію для обробки великих XML файлів
- Хостинг з достатнім обсягом пам'яті (рекомендовано від 128MB)
...
1. Після активації плагіна зайдіть у розділ "Оновлення XML" у меню адміністратора WordPress.
...
- **Імпорт товарів:** Перейдіть на вкладку "Імпорт XML" для завантаження і імпорту нових товарів з XML файлу.
...
- Оптимізована обробка великих XML файлів (до 10 000+ товарів).
- Пакетна обробка для зменшення використання пам'яті.
- Автоматичне видалення всіх налаштувань при деактивації плагіну.

## Оптимізація для продакшену

- Плагін використовує пакетну обробку для ефективного оновлення великої кількості товарів.
- Автоматично адаптується до доступних ресурсів сервера.
- Показує попередження, якщо ресурси сервера недостатні для оптимальної роботи.
- Регулярно звільняє пам'ять під час процесу оновлення.
- Надійна система відновлення після помилок.

## Видалення

При видаленні плагіну автоматично видаляються:
- Всі заплановані CRON задачі
- Всі налаштування та опції плагіну
...
3. **Налаштування**
   - Зайдіть у розділ "Оновлення XML" і вкажіть необхідні параметри.

4. **Запуск оновлення**
   - Натисніть "Update stock status" для ручного оновлення.
